...
def one_to_many(teachers, courses):
    return [(t.name, t.wage, c.name)
            for t in teachers
            for c in courses
            if t.id_course == c.id]
def many_to_many(teachers, courses):
    many_to_many_tmp = [(c.name, tc.id_course, tc.id_teacher)
                        for c in courses
                        for tc in teacherofcourse
                        if c.id == tc.id_course]
    return [(t.id, id_course)
...
def many_to_many(teachers, courses):
        many_to_many_temp = [(c.name, tc.id_course, tc.id_teacher)
                            ...
                            for tc in teacherofcourse
                            if c.id == tc.id_course]
        return [(t.name, t.wage, course_name)
                for course_name, id_course, id_teacher in many_to_many_temp
                for t in teachers
                if t.id == id_teacher]

def A1(courses, teachers) -> list:
    res1 = sorted(one_to_many(teachers, courses), key=itemgetter(2))
    return list(res1)
def A2(courses, teachers) -> list:
    res2_unsorted = []
    for c in courses:
        c_courses = list(
            filter(lambda i: i[2] == c.name, one_to_many(teachers, courses)))
        if len(c_courses) > 0:
...
def A3(courses, teachers):
    res3 = {}
    for c in courses:
        if 'физика' in c.name:
            c_courses = list(
                filter(lambda i: i[2] == c.name, many_to_many(teachers, courses)))
...
if __name__ == '__main__':
    print('Task А1')
    print(A1(courses, teachers))
    print('Task А2')
    print(A2(courses, teachers))
    print('Task А3')
    print(A3(courses, teachers))
